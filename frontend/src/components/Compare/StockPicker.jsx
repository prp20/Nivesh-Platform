...
import { addStockToCompare, removeStockFromCompare } from '../../store/slices/stockCompareSlice';
import stockService from '../../api/services/stockService';
import toast from 'react-hot-toast';

const StockPicker = () => {
    const dispatch = useDispatch();
    const { compareList } = useSelector((state) => state.stockCompare);
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            if (searchTerm.trim().length > 1) {
                setIsSearching(true);
                try {
                    const data = await stockService.searchStocks(searchTerm);
                            ...
                            {asset ? (
                                ...
                                >
                                    ...
                                    <div className="w-10 h-10 rounded-xl flex items-center justify-center border mb-4 transition-all duration-700" style={{ backgroundColor: `${accentColor}11`, borderColor: `${accentColor}22` }}>
                                        <span className="material-symbols-outlined text-xl" style={{ color: accentColor }}>insights</span>
                                    </div>
                                    <h4 className="text-[10px] font-label font-bold text-white leading-snug line-clamp-3 mb-1 px-2">{asset.symbol}</h4>
                                    <p className="font-label text-[8px] text-slate-500 tracking-[0.2em] uppercase font-black mb-4">{asset.name || 'Equity Asset'}</p>
                                    
                                    <button 
                                        onClick={() => dispatch(removeStockFromCompare(asset.symbol))}
                                        className="mt-auto px-4 py-2 rounded-lg bg-error/5 border border-error/10 hover:bg-error/20 transition-all flex items-center gap-2"
                                    >
            ...
            {compareList.length < 4 && (
                <div className="relative max-w-xl mx-auto">
                    <div className="relative group">
                        <input 
                            type="text" 
                            placeholder="INJECT STOCK ARTIFACT..."
                            value={searchTerm}
                            onChange={(e) => {
                                        ...
                                        {searchResults.slice(0, 10).map((stock) => (
                                            ...
                                            >
                                                <div className="flex flex-col gap-1">
                                                    <div className="text-sm font-headline font-bold text-white group-hover:text-primary transition-colors uppercase italic tracking-tight">{stock.symbol}</div>
                                                    <div className="text-[9px] text-slate-500 font-black tracking-[0.2em] uppercase font-label">{stock.name || stock.sector}</div>
                                                </div>
