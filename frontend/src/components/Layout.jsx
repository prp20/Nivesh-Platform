    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            ...
            if (trimmedQuery.length > 2) {
                setIsSearching(true);
                try {
                    console.log(`[Matrix Search] Initializing protocol for: ${trimmedQuery}`);
                    // Parameters: skip, limit, category, amc, subcategory, plan_type, order_by, search
    ...
    return (
        <header className="w-full h-20 top-0 sticky bg-[#0f1419]/90 backdrop-blur-2xl border-b border-white/5 shadow-[0_8px_32px_rgba(233,195,73,0.06)] flex justify-between items-center px-6 md:px-12 xl:px-24 2xl:px-32 z-50 transition-all duration-300">
            <div className="flex items-center gap-6 md:gap-12">
                <Link to="/dashboard" className="text-xl md:text-2xl font-headline font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-[#e9c349] to-[#9d7e00] hover:scale-[1.02] transition-transform uppercase italic">Nivesh Elite</Link>
            ...
            <nav className="flex-1 space-y-1">
                {navItems.map((item) => (
                    <NavLink
                        ...
                        to={item.path}
                        className={({ isActive }) => `
                            flex items-center justify-between py-5 px-8 font-body text-[11px] tracking-[0.2em] uppercase font-bold transition-all duration-300 group
                            ${isActive 
